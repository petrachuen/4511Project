...
def constructBayesNet(gameState: hunters.GameState):
    """
    Construct an empty Bayes net in accordance with the structure given in Figure 1
    ...
    MAX_NOISE = 7 # Don't change

    #print(X_RANGE)

    variables = [GHOST0, GHOST1, OBS0, PAC, OBS1]
    ...
    variableDomainsDict[PAC] = gridList
    variableDomainsDict[GHOST0] = gridList
    variableDomainsDict[GHOST1] = gridList
    variableDomainsDict[OBS1] = ditsList
    variableDomainsDict[OBS0]= ditsList
...
def inferenceByEnumeration(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict):
    ...
    queryVariables: A list of the variables which are unconditioned in
                    the inference query.
    evidenceDict:   An assignment dict {variable : value} for all
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for e in eliminationOrder:
            factor, resultFac = joinFactorsByVariable(factor, e)

            if (len(resultFac.unconditionedVariables()) != 1):
                temp = eliminate(resultFac,e)
                factor.append(temp)

        returnFac = normalize(joinFactors(factor))
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        #pre-declared cause this girl is changing like a dc weather
        sum = (self.total())
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        return busters.getObservationProbability(noisyDistance, manhatDist)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for i in self.allPositions:
            pr = self.getObservationProb(observation, gameState.getPacmanPosition(), i, self.getJailPosition())
            
            #update
            dist[i] = pr * self.beliefs[i]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            oldPostBelief = self.beliefs[oldPos]

            for npd in newPosDist:
                dist[npd] = dist[npd] + oldPostBelief * newPosDist[npd]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        Use self.legalPositions for the legal board positions where particle could be located.
        ...
        while par < self.numParticles:
            ...
            for legalPos in self.legalPositions:
                #yeet the particle in the position
                if par < self.numParticles:
                    par += 1
    ...
    def getBeliefDistribution(self):
        ...
        (Which is not the same thing as a normal distribution.)
        """
        "*** YOUR CODE HERE ***"

...
#        GameState.explored.add(self)
    ...
    def generateSuccessor(self, agentIndex, action):
        ...
        PacmanRules.checkDeath(state, agentIndex)

        # Book keeping
        state.data._agentMoved = agentIndex
        state.data.score += state.data.scoreChange
        state.data.ghostEatenRecord += state.data.ghostEatenChange
        GameState.explored.add(self)
    ...
    def getGhostState(self, agentIndex):
        #for i in (self.data.agentStates):
            #print(i)
    ...
    def hasFood(self, x, y):
        return self.data.food[x][y] > 0

    def hasWall(self, x, y):
        return self.data.layout.walls[x][y]
    
    def setGhostNotLiving(self, index):
        self.livingGhosts[index-1] = False
    ...
    def initialize(self, layout, numGhostAgents=1000):
        ...
        self.livingGhosts = [True for i in range(numGhostAgents)]
        #print(self.livingGhosts)
...
class ClassicGameRules:
    ...
    def win(self, state, game):
        if not self.quiet:
            print("Ghost capture all food! Score: %d" % state.data.score)
        game.gameOver = True

    def lose(self, state, game):
        if not self.quiet:
            print("Pacman eat all Ghost! Score: %d" % state.data.score)
        game.gameOver = True
...
class PacmanRules:
    ...
    def checkDeath(state, agentIndex):
        ...
        if agentIndex != 0:
            ...
            if PacmanRules.canKill(pacmanPosition, ghostPosition):  
                PacmanRules.collide(state, ghostState, agentIndex)
    checkDeath = staticmethod(checkDeath)

    def collide(state, ghostState, agentIndex):
        state.data.ghostEatenChange.append(agentIndex)
        state.setGhostNotLiving(agentIndex)

        state.data.scoreChange -= 50
            
        if state.livingGhosts.count(True) == 0: #== 4: #True:#not state.data._win:
...
class GhostRules:
    ...
    def applyAction(state, action, ghostIndex):
        ...
        if manhattanDistance(nearest, next) <= 0.5:
            # Remove food
            if state.livingGhosts[ghostIndex -1]:
    ...
    def consume(position, state):
        ...
        if state.data.food[x][y] > 0:
            ...
            #print (state.data.food[x][y])
            state.data.scoreChange += 10
            state.data.food = state.data.food.copy()
            state.data.food[x][y] -= 1 #= False
            #print (state.data.food[x][y])
            if state.data.food[x][y] == 0:
...
def readCommand(argv):
    ...
    parser.add_option('-p', '--pacman', dest='pacman',
                      ...
                      metavar='TYPE', default= 'PacmanRandomAgent')# 'PacmanRandomAgent') #'PacmanQAgent')#KeyboardAgent')
    ...
    parser.add_option('-g', '--ghosts', dest='ghost',
                      ...
                      metavar='TYPE', default= 'QLearningAgent') #'DirectionalGhost'  QLearningAgent, RandomGhost)
    ...
    for moduleDir in pythonPathDirs:
        ...
        for modulename in moduleNames:
            ...
            if pacman in dir(module):
                #print("hi")
                """if nographics and modulename == 'pacmanRandomAgents.py': #keyboardAgents.py':
                    raise Exception(
